...
/* =========================================================
   ...
   ========================================================= */
/* Rough byte-size estimate of everything the app has stored in
   localStorage + sessionStorage (used for the "Clear Cache" label). */
function estimateWebStorageBytes() {
  let total = 0;
  try {
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      const val = localStorage.getItem(key) || "";
      total += (key ? key.length : 0) + val.length;
    }
  } catch (e) {
    logDebug("localStorage read failed: " + e.message);
  }
  try {
    ...
    logDebug("sessionStorage read failed: " + e.message);
  }
  return total;
}
...
/* Reads the real cache size (Cache Storage API via navigator.storage.estimate()
   when available, falling back to a localStorage/sessionStorage byte count)
   and updates the label shown under "Clear Cache". */
async function updateCacheSizeLabel() {
  const label = document.getElementById("cache-size-label");
  if (!label) return;

  let bytes = estimateWebStorageBytes();
  ...
  if (navigator.storage && navigator.storage.estimate) {
    try {
      ...
      logDebug("storage.estimate failed: " + e.message);
    }
  }

  label.textContent = `${formatCacheSize(bytes)} • Free up space`;
}

/* Actually clears app cache: localStorage, sessionStorage, and any
   Cache Storage (service-worker) entries — then refreshes the label. */
async function clearAppCache() {
  const row = document.getElementById("clear-cache-row");
  const label = document.getElementById("cache-size-label");
  if (row) row.classList.add("cache-clearing");
  if (label) label.textContent = "Clearing…";

  try {
    localStorage.clear();
  } catch (e) {
    logDebug("localStorage.clear failed: " + e.message);
  }
  try {
    sessionStorage.clear();
  } catch (e) {
    logDebug("sessionStorage.clear failed: " + e.message);
  }
  ...
  if (window.caches && caches.keys) {
    try {
      ...
      logDebug("caches.delete failed: " + e.message);
    }
  }

  if (row) row.classList.remove("cache-clearing");
  await updateCacheSizeLabel();
  showToast("Cache cleared");
}

/* =========================================================
   Settings — About & Support links
   ========================================================= */
/* TODO: replace with your real Privacy Policy URL */
const PRIVACY_POLICY_URL = "https://example.com/PLACEHOLDER-privacy-policy";
/* TODO: replace with your real Play Store listing URL */
const PLAY_STORE_URL = "https://play.google.com/store/apps/details?id=PLACEHOLDER.PACKAGE.NAME";
/* TODO: replace with your real X (Twitter) profile URL */
const X_PROFILE_URL = "https://x.com/PLACEHOLDER_HANDLE";
...
/* Web Share API with a clipboard-copy fallback for browsers that don't support it. */
async function shareApp() {
  const shareData = {
    title: "DepthX",
    text: "Check out DepthX — stunning depth & parallax wallpapers!",
  ...
  if (navigator.share) {
    try {
      ...
      // user closed/cancelled the native share sheet — nothing to do
    }
    return;
  }
...
/* =========================================================
   Settings — Connect With Us
   ========================================================= */
function openXProfile() {
  window.open(X_PROFILE_URL, "_blank", "noopener");
}
