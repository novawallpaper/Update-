...
/* =========================================================
   ...
   ---------------------------------------------------------
   This is the ONLY place category structure is defined.
   Homepage shows just the 17 primary categories below (each
   `key`/`title`). Each primary category lists the subcategory
   names that belong to it.

   SCALABLE BY DESIGN — to add a new subcategory later:
     1) add its name into the right primary's `subcategories` array
     2) tag wallpapers with that subcategory name in WALLPAPERS'
        `category` field (see WALLPAPERS below)
   That's it — counts, category-detail filtering and search all
   read from this config automatically. No other code changes
   needed.

   A subcategory may legitimately appear under more than one
   primary category (e.g. "Dragon" is both an Iconic Legends and
   an Anime World theme) — a wallpaper tagged "Dragon" will then
   correctly surface under both.
   ========================================================= */
const CATEGORIES = [
  { key: "Iconic Legends", title: "Iconic Legends", img: "https://novawallpaper.github.io/depthx-app/pexels-koyeldey-31179659.jpg",
    subcategories: ["Marvel", "DC", "Superheroes", "Anime Heroes", "Samurai", "Ninja", "Dragon", "Gaming Legends"] },

  { key: "Nature Scapes", title: "Nature Scapes", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30018099.jpg",
    subcategories: ["Nature", "Mountains", "Rivers", "Forests", "Beaches", "Ocean", "Waterfalls", "Desert", "Sky", "Sunset", "Rain", "Winter", "Wildlife"] },

  { key: "Animal Kingdom", title: "Animal Kingdom", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30936352.jpg",
    subcategories: ["Animals", "Cats", "Dogs", "Birds", "Wildlife"] },

  { key: "Cute Zone", title: "Cute Zone", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-29986988.jpg",
    subcategories: ["Cute", "Aesthetic", "Love", "Girls"] },

  { key: "Anime World", title: "Anime World", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30843199.jpg",
    subcategories: ["Anime", "Manga", "Fantasy", "Dragon", "Samurai", "Ninja"] },

  { key: "Auto Garage", title: "Auto Garage", img: "https://novawallpaper.github.io/depthx-app/pexels-koyeldey-31179659.jpg",
    subcategories: ["Cars", "Bikes", "Vehicles", "Aircraft", "Boats", "Trains"] },

  { key: "Gaming Arena", title: "Gaming Arena", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-29986988.jpg",
    subcategories: ["Gaming", "Cyberpunk", "Sci-Fi", "Technology"] },

  { key: "Space Universe", title: "Space Universe", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30018099.jpg",
    subcategories: ["Space", "Galaxy", "Universe", "Planets"] },

  { key: "Art Studio", title: "Art Studio", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30843199.jpg",
    subcategories: ["Abstract", "Art", "Illustration", "Pattern", "Typography", "Wall Art", "Mandala", "Vintage", "3D", "AI Generated"] },

  { key: "Minimal Zone", title: "Minimal Zone", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30936352.jpg",
    subcategories: ["Minimal", "Dark", "AMOLED", "Black", "Gradient", "Textures"] },

  { key: "Urban Life", title: "Urban Life", img: "https://novawallpaper.github.io/depthx-app/pexels-koyeldey-31179659.jpg",
    subcategories: ["City", "Architecture", "Photography", "Travel", "Night"] },

  { key: "Entertainment Hub", title: "Entertainment Hub", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-29986988.jpg",
    subcategories: ["Movies", "Music", "Sports"] },

  { key: "Fantasy Realm", title: "Fantasy Realm", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30018099.jpg",
    subcategories: ["Fantasy", "Horror", "Fire", "Ice", "Lightning", "Skull"] },

  { key: "Spiritual World", title: "Spiritual World", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30843199.jpg",
    subcategories: ["Religion", "Islamic", "Hindu", "Christian"] },

  { key: "Festival Vibes", title: "Festival Vibes", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30936352.jpg",
    subcategories: ["Festival", "Diwali", "Holi", "Eid", "Ramadan", "Christmas", "Valentine", "Independence Day"] },

  { key: "Color Collection", title: "Color Collection", img: "https://novawallpaper.github.io/depthx-app/pexels-koyeldey-31179659.jpg",
    subcategories: ["Black", "White", "Red", "Blue", "Green", "Yellow", "Orange", "Purple", "Pink", "Brown", "Grey", "Cyan", "Teal", "Gold", "Silver", "Multicolor"] },

  { key: "Premium Picks", title: "Premium Picks", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-29986988.jpg",
    subcategories: ["Premium", "Featured", "Trending", "Popular", "New", "Live Wallpapers", "HD", "4K", "8K"] },
];

// Kept so any old code referencing COLLECTIONS keeps working.
const COLLECTIONS = CATEGORIES;

/* Reverse lookup, built automatically from CATEGORIES above:
   subcategory name (lowercase) -> array of primary category keys it belongs to. */
const SUBCAT_TO_PRIMARIES = {};
CATEGORIES.forEach((cat) => {
  cat.subcategories.forEach((sub) => {
    const key = sub.toLowerCase();
    if (!SUBCAT_TO_PRIMARIES[key]) SUBCAT_TO_PRIMARIES[key] = [];
    SUBCAT_TO_PRIMARIES[key].push(cat.key);
  });
});

function primariesForSubcategory(subcategory) {
  return SUBCAT_TO_PRIMARIES[(subcategory || "").toLowerCase()] || [];
}

function wallpaperBelongsToPrimary(w, primaryKey) {
  return primariesForSubcategory(w.category).includes(primaryKey);
}

/* ---------------- Sample wallpaper data ----------------
   `category` MUST be one of the subcategory names listed inside
   CATEGORIES above (case-insensitive) — that's what buckets a
   wallpaper into its primary category automatically.

   `keywords` (optional array) lets a wallpaper be found by extra
   search terms that aren't the subcategory name itself — e.g. a
   wallpaper tagged category:"Marvel" with
   keywords:["spiderman","spider-man","avengers"] will show up when
   a user searches "spiderman", even though the category is "Marvel". */
const WALLPAPERS = [
  ...
  { id: "w3", title: "Wallpaper 3", time: "23:47", category: "Cyberpunk", premium: true, tag: "fire", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30018099.jpg" },
  { id: "w4", title: "Wallpaper 4", time: "18:05", category: "Cyberpunk", premium: false, tag: "gift", img: "https://novawallpaper.github.io/depthx-app/pexels-steve-30843199.jpg" },
...
function renderCollections() {
  ...
  el.innerHTML = CATEGORIES.map((c) => {
    const count = WALLPAPERS.filter((w) => wallpaperBelongsToPrimary(w, c.key)).length;
    return `
...
function renderCategoryDetailGrid() {
  let list = WALLPAPERS.filter((w) => wallpaperBelongsToPrimary(w, state.catDetailCategory));
...
function getFilteredWallpapers() {
  ...
  if (state.activeCategory !== "All") {
    list = list.filter((w) => wallpaperBelongsToPrimary(w, state.activeCategory));
  }
  ...
  if (state.searchQuery) {
    ...
    list = list.filter((w) => {
      const haystack = [
        w.title,
        w.category,
        ...primariesForSubcategory(w.category),
        ...(w.keywords || []),
      ]
        .join(" ")
        .toLowerCase();
